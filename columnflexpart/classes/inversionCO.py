...
# passt da es so noch in predictions_CO steht
...
class InversionCO():
    def __init__(
        ...
        self.flux, self.flux_errs = self.get_flux() # adjust to get GFED flux !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def get_GFED_flux(self, bio_only=False, no_bio=False) -> tuple[xr.DataArray,xr.DataArray]:
        """Loads fluxes and its errors and coarsens to parameters given in __init__
        ...
        while date < pd.to_datetime(self.stop):
            date += datetim.timedelta(days = 1)
            dates.append(pd.to_datetime(date))
        fluxes = xr.DataArray(data = np.zeros((len(dates), len(total_flux.latitude.values), len(total_flux.longitude.values))), dims = ['time', 'latitude', 'longitude'])
        count = 0
        for i,dt in enumerate(dates): 
    ...
    def get_regression(
        ...
        if not xerr is None:
            ...
            #self.flux_errs_flat = self.flux_errs.values.flatten()
        ...
        if with_prior:
            ...
            self.reg = bayesinverse.Regression(
                ...
                y_covariance = (concentration_errs*1e-9)**2, 
                alpha = alpha
            )
        else:
            self.reg = bayesinverse.Regression(
    ...
    def add_plot_on_map(ax: plt.Axes, xarr: xr.DataArray, map_kwargs: dict = dict(), **kwargs):
        ...
        FlexDatasetCO.add_map(ax = ax, **map_kwargs)
        return ax
    ...
    def plot_total_emission(
        ...
        **kwargs
        ...
        if ax is None:
            fig, ax = FlexDatasetCO.subplots(figsize=figsize)
        else:
            fig = ax.get_figure()
    ...
    def plot_all_total_emissions(
        ...
        **kwargs
        ...
        if axes is None:
            fig, axes = FlexDatasetCO.subplots(1,3, figsize=figsize, **kwargs)
        else:
...
class InversionGrid(InversionCO):
    def __init__(
        self, 
        result_path: Pathlike,
        month: str, 
...
class InversionBioclassCO(InversionCO):
    def __init__(self, 
        result_path: Pathlike,
