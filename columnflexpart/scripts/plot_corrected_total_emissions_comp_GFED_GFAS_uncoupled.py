...
def get_CO2_masked_total_emissions(datapath_inversion, datapathmasks,name_mask_before_week,  spatial_results_name_before_week, savename):
    ...
    ds = xr.open_dataarray(datapath_inversion+spatial_results_name_before_week+'48.nc')#str("{:e}".format(alpha))+'_CO2_spatial_results_week_48.nc')
    ...
    for week in range(49,53): 
        ds = xr.open_dataarray(datapath_inversion+spatial_results_name_before_week+str(week)+'.nc')
    ...
    ds = xr.open_dataarray(datapath_inversion+spatial_results_name_before_week+'1.nc')
...
def get_CO_masked_total_emissions(datapath_inversion, datapathmasks, name_mask_before_week, spatial_result_name_before_week, savename):
    ...
    masked_area = []
    #alpha =3e-2
    ds = xr.open_dataarray(datapath_inversion+spatial_result_name_before_week+'48.nc')
    #print(ds)
    ...
    for week in range(49,53): 
        ds = xr.open_dataarray(datapath_inversion+spatial_result_name_before_week+str(week)+'.nc')
        mask = xr.open_dataset(datapathmasks+name_mask_before_week+str(week)+'.nc')
        #mask = mask.rename({'Lat': 'latitude', 'Long': 'longitude', 'bioclass': '__xarray_dataarray_variable__'})
        Area_lat_long = np.zeros((len(dfArea['Area'][(dfArea['Lat']>=mask.latitude.min().values)&(dfArea['Lat']<=mask.latitude.max().values)][:]),len(mask.longitude.values) ))
        for i in range(len(mask.longitude.values)): 
        ...
        #gC/m^2/s für eine Woche 
        sums.append( gdf['CO2'].mean()*10**(-6)*masked_Area*10**(-12)*(7*24*60*60))
    ds = xr.open_dataarray(datapath_inversion+spatial_result_name_before_week+'1.nc')
...
def multiply_masks_with_GFED(datapathmasks,masked_area, name_mask, year, month, molecule,datapath_and_name_GFED, mask_monthly = False,just_land_mask = False): 
    ...
    print(GFED)
    print(GFED['total_emission'].mean()*factor*(7692024*10**6)*12)
    ...
    if just_land_mask == False: 
        for idx,week in enumerate([48,49,50,51,52,1]): # 26, 146
            ...
            maskedGFED = xrGFED.total_emission*mask.__xarray_dataarray_variable__#(7692024*10**6)
            print(maskedGFED)
            if week == 48: 
                weekly_mean_values.append(maskedGFED.sum(['latitude', 'longitude'])*factor*(1/31))#*masked_area[idx])
            elif week == 1: 
                weekly_mean_values.append(maskedGFED.sum(['latitude', 'longitude'])*factor*(2/31))#*masked_area[idx])
            else: 
                weekly_mean_values.append(maskedGFED.sum(['latitude', 'longitude'])*factor*(7/31))#*masked_area[idx])
    else: 
...
def multiply_masks_with_GFAS(datapathmasks,masked_area,name_mask, year, month, molecule,datapath_and_name_GFAS, just_land_mask = False): 
    ...
    weekly_mean_values = []
    print((GFAS[variable]* np.cos(np.deg2rad(GFAS["latitude"][:]))* 111319* 111000*10**(-2)*10**(-9)*30.4*60*60*24*random_factor).sum())
    for idx,week in enumerate([48,49,50,51,52]):
        ...
        if just_land_mask == False: 
            mask = xr.open_dataset(datapathmasks+name_mask+str(week)+'.nc')
            #mask = mask.rename({'Lat': 'latitude', 'Long': 'longitude', 'bioclass': '__xarray_dataarray_variable__'})
            maskedGFAS = GFAS_week[variable]*10**(-9)*mask.__xarray_dataarray_variable__* np.cos(np.deg2rad(GFAS_week["latitude"][:]))* 111319* 111000*10**(-2)#* 111319* 111000*10**(-2)
            #maskedGFAS = maskedGFAS*masked_area[idx]#* np.cos(np.deg2rad(maskedGFAS["latitude"][:]))
        #else: 
        #    mask = xr.open_dataset(datapathmasks+'Mask_AU_land_bioclasses_based.nc')
    ...
    for week in [1]: 
        ...
        if just_land_mask == False: 
            mask = xr.open_dataset(datapathmasks+name_mask+str(week)+'.nc')
            #mask = mask.rename({'Lat': 'latitude', 'Long': 'longitude', 'bioclass': '__xarray_dataarray_variable__'})
            maskedGFAS = GFAS_week[variable]*mask.__xarray_dataarray_variable__*10**(-2)*10**(-9)
        else: 
            mask = xr.open_dataset(datapathmasks+'Mask_AU_land_bioclasses_based.nc')
            maskedGFAS = GFAS_week[variable]#*mask.bioclass
        #maskedGFAS = GFAS_week.co2fire*mask.__xarray_dataarray_variable__
        
        weekly_mean_values.append(maskedGFAS.mean()*(2*24*60*60)*random_factor*masked_area[5])
    return weekly_mean_values
...
def plot_CO2_GFAS_GFED_posterior_prior(savepath, savename,weekly_mean_GFAS, weekly_mean_GFED, posterior, prior, masked_area, prior_error, post_error):
    ...
    fig, ax = plt.subplot_mosaic([['lh', 'r'],['lb', 'r']],  gridspec_kw={'width_ratios': [5,1],'height_ratios':[5,1], 'hspace' : 0}, figsize = (7,6), constrained_layout = True)
    ...
    ax['lh'].plot(weeks, prior,marker = 'o',color = 'orange')# orange
    eb = ax['lh'].errorbar(weeks, prior, yerr = np.array(prior_error), marker = 'o', capthick=2, capsize=4,color = 'orange')
    #eb[-1][0].set_linestyle('-.') 
    ax['lh'].plot(weeks, posterior,marker = 'o',color = 'coral')#,label='Posterior')# darkorange
    ...
    eb = ax['r'].errorbar(0.6, np.array(prior).sum(), yerr = np.array(prior_error).sum(),linestyle = 'dashed',marker = 'o', capthick=3, capsize=4,color = 'k')
    #eb[-1][0].set_linestyle('-.')
    ax['r'].bar(1.2,posterior.sum(), width = 0.6,label='Posterior', color = 'coral')
    eb2 = ax['r'].errorbar(1.2, np.array(posterior).sum(), yerr = np.array(post_error).sum(),marker = 'o', capthick=3, capsize=4, color = 'k')
    ...
    #ax2.legend()
    #ax.legend()
    fig.legend(bbox_to_anchor=(0.25,0.71,0.2,0.2))
    plt.show()
    fig.savefig(savepath+savename, bbox_inches = 'tight',facecolor = 'w', dpi = 300)
...
def plot_CO_GFAS_GFED_posterior_prior(savepath, savename, weekly_mean_GFAS, weekly_mean_GFED, posterior, prior, masked_area, prior_error, post_error):
    weeks = [48,49,50,51,52,53] 
    plt.rcParams.update({'font.size':20})   
    fig,ax = plt.subplot_mosaic([['lh', 'r'],['lb', 'r']],  gridspec_kw={'width_ratios': [5,1],'height_ratios':[5,1], 'hspace' : 0}, figsize = (7,6), constrained_layout = True)#10,6
    plt.subplots_adjust(left=0.1,
                        bottom=0.1,
                        right=0.9,
                        top=0.9,
...
def get_prior_weekly(masked_area, datapath_inversion,filename): 
    prior_weekly = []
    prior = xr.open_dataset(datapath_inversion+filename+str(48))#'prior_spatially_week_'+str(48))
    ...
    for idx,week in enumerate([49,50,51,52]): 
        prior = xr.open_dataset(datapath_inversion+filename+str(week))#'prior_spatially_week_'+str(week))
        prior_weekly.append(prior.__xarray_dataarray_variable__.mean()*10**(-18)*masked_area[idx+1]*(7*24*60*60))
    prior = xr.open_dataset(datapath_inversion+filename+str(1))#'prior_spatially_week_'+str(1))
    prior_weekly.append(prior.__xarray_dataarray_variable__.mean()*10**(-18)*masked_area[5]*(2*24*60*60))

    return prior_weekly


GFED_CO = '/work/bb1170/RUN/b382105/Dataframes/GFEDs/GDF_CO_AU20192020_regr1x1.pkl'# GDF_AU_CO_2019_2020.pkl
GFED_CO2 =  '/work/bb1170/RUN/b382105/Dataframes/GFEDs/GDF_CO2_AU20192020_regr1x1.pkl'#GDF_CO2_AU20162021.pkl
...
savepath_CO = '/work/bb1170/RUN/b382105/Flexpart/TCCON/output/one_hour_runs/CO2/splitted/Images_CO/Setup_gridded/'#Ecosystems_AK_split_with_4/'
savepath_CO2 = '/work/bb1170/RUN/b382105/Flexpart/TCCON/output/one_hour_runs/CO2/splitted/Images/Setup_gridded/'#Ecoregions_split_4/flat_error_area_scaled/'#
datapathmask = '/work/bb1170/RUN/b382105/Flexpart/TCCON/output/one_hour_runs/CO2/splitted/Images/Setup_gridded/total_emissions_and_ratios_check/'#Ecoregions_split_4/flat_error_area_scaled/total_emissions/'#
...
#datapath = '/work/bb1170/RUN/b382105/Flexpart/TCCON/output/one_hour_runs/CO2/splitted/Images/Setup_gridded/'
name_mask = '1.00e-05_CO2_1.00e-05_CO_mask_ak_threshold_0.001and_co_5e-1_week_'
#name_mask = '3.00e-02_CO2_3.00e-02_CO_mask_ak_threshold_0.001and_co_5e-1_week_'
alpha = 1e-5
name_spatial_results_before_week_CO2 = str("{:e}".format(alpha))+'_CO2_spatial_results_week_'
name_spatial_results_before_week_CO = str("{:e}".format(alpha))+'_CO_spatial_results_week_'
weeks = [48,49,50,51,52,53] 
# gridded masked area: 
masked_area = [3.04356787e+10, 0,2.20228191e+11, 7.48989837e+11, 1.30073375e+12, 6.56704376e+11]

masked_area = get_CO2_masked_total_emissions(savepath_CO2, datapathmask, name_mask, name_spatial_results_before_week_CO2,str("{:e}".format(1e-5))+'_total_emission_masked_with_area_weekly.nc' )
get_CO_masked_total_emissions(savepath_CO, datapathmask, name_mask, name_spatial_results_before_week_CO,str("{:e}".format(1e-5))+'_total_emission_masked_with_area_weekly.nc')
CO_posterior = xr.open_dataarray(savepath_CO+str("{:e}".format(1e-5))+'_total_emission_masked_with_area_weekly.nc')
CO2_posterior = xr.open_dataarray(savepath_CO2+str("{:e}".format(1e-5))+'_total_emission_masked_with_area_weekly.nc')

CO_prior = get_prior_weekly(masked_area, '/work/bb1170/RUN/b382105/Flexpart/TCCON/output/one_hour_runs/CO2/splitted/Images_CO/Setup_gridded/test/','prior_spatially_week_')
CO2_prior = get_prior_weekly(masked_area,'/work/bb1170/RUN/b382105/Flexpart/TCCON/output/one_hour_runs/CO2/splitted/Images/Setup_gridded/','prior_spatially_week_')
...
for threshold in [1e-3]:
    for idx, week in enumerate([48,49,50,51,52,1]): 
        ...
        prior_errors_CO = xr.open_dataarray(savepath_CO+str(alpha)+'_CO_spatial_prior_error_week'+str(week)+'.nc')
        posterior_errors_CO = xr.open_dataarray(savepath_CO+str(alpha)+'_CO_spatial_posterior_error_week'+str(week)+'.nc')
        prior_errors_CO2 = xr.open_dataarray(savepath_CO2+str(alpha)+'_CO2_spatial_prior_error_week'+str(week)+'.nc')
        posterior_errors_CO2 = xr.open_dataarray(savepath_CO2+str(alpha)+'_CO2_spatial_posterior_error_week'+str(week)+'.nc')
