...
def get_CO2_masked_total_emissions():
        # for CO2 masked save
    datapath_inversion = '/work/bb1170/RUN/b382105/Flexpart/TCCON/output/one_hour_runs/CO2/splitted/Images/Setup_gridded/'
...
def multiply_masks_with_GFED(datapathmasks,masked_area, name_mask, year, month, molecule,datapath_and_name_GFED, mask_monthly = False,just_land_mask = False): 
    ...
    if just_land_mask == False: 
        for idx,week in enumerate([48,49,50,51,52,1]):
            ...
            print(mask)
            maskedGFED = xrGFED.total_emission*masked_area[idx]/(7692024*10**6)*mask.__xarray_dataarray_variable__
            print(maskedGFED)
            if week == 48: 
...
def multiply_masks_with_GFAS(datapathmasks,masked_area,name_mask, year, month, molecule,datapath_and_name_GFAS, just_land_mask = False): 
    ...
    for idx,week in enumerate([48,49,50,51,52]):
        ...
        #
        if week == 48: 
            weekly_mean_values.append(maskedGFAS.mean()*masked_area[0]*10**(-9)*factor*(1*24*60*60))
        else: 
            weekly_mean_values.append(maskedGFAS.mean()*masked_area[idx]*10**(-9)*factor*(7*24*60*60))
    ...
    for week in [1]: 
        ...
        weekly_mean_values.append(maskedGFAS.mean()*masked_area[5]*10**(-9)*factor*(2*24*60*60))
    return weekly_mean_values
...
plot_CO_GFAS_GFED_posterior_prior(savepath_CO, 'Comparison_GFED_GFAS_PRIOR_POSTERIOR_areas_considered.png', GFAS_weekly_CO, GFED_weekly_CO, CO_posterior, CO_prior, masked_area)
plot_CO2_GFAS_GFED_posterior_prior(savepath_CO2, 'Comparison_GFED_GFAS_PRIOR_POSTERIOR_areas_considered.png', GFAS_weekly_CO2, GFED_weekly_CO2, CO2_posterior, CO2_prior, masked_area)