...
def plot_averaging_kernel(Inversion, alpha, class_num, week_num,savepath, plot_spatially, weekly = False):
    ...
    if plot_spatially ==  True and weekly == False: 
        ...
        #print(len(ak_xr.ak))
        #print(ak_xr.ak)       
        ak_spatial = Inversion.map_on_grid_without_time_coord(ak_xr.ak, len(ak_xr.ak))
        return ak_spatial
        print(ak_spatial)
...
def plotFireMonthlyMeanSpatially(savepath, syear, smonth, gdf, Inversion, savefig=True):
    ...
    ak_equal = ak_to_plot.where(ak_to_plot>0, 1, 0)#*1
    print(ak_equal)
    ...
    #ak_equal.plot(norm = LogNorm(vmin = 1e-3),x='longitude', y = 'latitude', ax = ax, cmap = 'Greys', add_colorbar=False)
    print('ak_plotted')
    
    foo2.plot.pcolormesh(x='Long', y = 'Lat',extend = 'max', cmap = cmap0, vmax = 0.2,ax = ax, cbar_kwargs=dict(orientation='vertical',
                      shrink=0.75, label='CO$_2$ Emissions [TgC/month]', ticks = np.arange(0,0.3,0.1)))#ax= ax,  transform = ccrs.PlateCarree() cmap = 'OrRd',
    print('GFED plotted')
    ...
    plt.savefig(savepath+"GFED_AK_CO2_AU_contour_iso_line_ak_1e-3.png")
